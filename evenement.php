...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_SESSION['user_id'])) {
    ...
    header("Location: login.php?redirect=" . urlencode("evenement.php?id=$id"));
    exit;
}
...
// Fetch evenement
$st = mysqli_prepare($conn, "SELECT * FROM evenement WHERE id = ? LIMIT 1");
...
// Fetch avis
$avis_list = [];
$st_avis = mysqli_prepare($conn, "SELECT a.*, u.nom as user_nom FROM avis a JOIN utilisateur u ON a.utilisateur_id = u.id WHERE a.evenement_id = ? ORDER BY a.created_at DESC");
if ($st_avis) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_SESSION['user_id'])) {
    ...
    if (empty($date)) {
        ...
        $stmt = mysqli_prepare($conn, "INSERT INTO reservations (utilisateur_id, evenement_id, date_debut, nb_personnes, statut) VALUES (?, ?, ?, ?, 'en_attente')");
        if ($stmt) {
            ...
            $order_error = 'Erreur lors de l\'inscription.';
        }
    }
}
...
function formatImagePath($path) {
    ...
    if (strpos($path, 'uploads/evenement/') === 0) return $path;
    return 'uploads/evenement/' . ltrim($path, '/');
}
...
<div class="product-container">
  <div class="left-column">
    ...
    <div class="description-block">
      ...
      <?php if(!empty($inclus)): ?>
      <h3 style="margin-top:32px;">Au programme</h3>
  ...
  <div class="booking-card">
    ...
    <form method="post" action="evenement.php?id=<?= $id ?>">
      ...
      <div class="form-group">
        ...
        <textarea name="message" class="input-field" rows="3" placeholder="Questions sur l'événement..."></textarea>
      </div>
