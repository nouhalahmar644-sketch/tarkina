...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_SESSION['user_id'])) {
    ...
    header("Location: login.php?redirect=" . urlencode("hebergement.php?id=$id"));
    exit;
}
...
// Fetch hebergement
$st = mysqli_prepare($conn, "SELECT * FROM hebergement WHERE id = ? LIMIT 1");
...
// Use localisation or region column for breadcrumb
$region_name = !empty($item['region']) ? $item['region'] : (!empty($item['localisation']) ? $item['localisation'] : 'Accueil');

// Fetch avis
$avis_list = [];
$st_avis = mysqli_prepare($conn, "SELECT a.*, u.nom as user_nom FROM avis a JOIN utilisateur u ON a.utilisateur_id = u.id WHERE a.hebergement_id = ? ORDER BY a.created_at DESC");
if ($st_avis) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_SESSION['user_id'])) {
    ...
    if (empty($date_arrivee) || empty($date_depart)) {
        $order_error = 'Veuillez sélectionner les dates.';
    } elseif (strtotime($date_depart) <= strtotime($date_arrivee)) {
        $order_error = 'La date de départ doit être après la date d\'arrivée.';
    } else {
        $user_id = $_SESSION['user_id'];
        
        // Insert reservation
        $nb_pers = isset($_POST['personnes']) ? (int)$_POST['personnes'] : 1;
        $stmt = mysqli_prepare($conn, "INSERT INTO reservations (utilisateur_id, logement_id, date_debut, date_fin, nb_personnes, statut) VALUES (?, ?, ?, ?, ?, 'en_attente')");
        if ($stmt) {
            mysqli_stmt_bind_param($stmt, 'iissi', $user_id, $id, $date_arrivee, $date_depart, $nb_pers);
...
function formatImagePath($path) {
    ...
    if (strpos($path, 'uploads/hebergement/') === 0) return $path;
    return 'uploads/hebergement/' . ltrim($path, '/');
}
...
<div class="product-container">
  ...
  <div class="booking-card">
    ...
    <form method="post" action="hebergement.php?id=<?= $id ?>">
      <input type="hidden" name="id" value="<?= $id ?>">
      
      <div class="form-row">
        <div>
            <label class="form-label">Date arrivée</label>
            <input type="date" name="date_arrivee" class="input-field" required>
        </div>
        <div>
            <label class="form-label">Date départ</label>
            <input type="date" name="date_depart" class="input-field" required>
        </div>
      </div>
      ...
      <div class="form-group">
        ...
        <textarea name="message" class="input-field" rows="3" placeholder="Demandes spéciales..."></textarea>
      </div>
