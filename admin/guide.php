...
// ---------- Upload directory ----------
$uploadBase = realpath(__DIR__ . '/..') . DIRECTORY_SEPARATOR . 'uploads' . DIRECTORY_SEPARATOR . 'guides';
...
$flashSuccess = '';
$flashError   = '';
if (!empty($_SESSION['guide_flash_success'])) { $flashSuccess = (string)$_SESSION['guide_flash_success']; unset($_SESSION['guide_flash_success']); }
if (!empty($_SESSION['guide_flash_error']))   { $flashError   = (string)$_SESSION['guide_flash_error'];   unset($_SESSION['guide_flash_error']); }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $redirect  = 'guide.php?q=' . urlencode($qReturn) . '&page=' . $pgReturn;

    // DELETE
    if ($action === 'delete') {
        if ($guideId <= 0) {
            $_SESSION['guide_flash_error'] = 'Guide invalide.';
        } else {
            $st = mysqli_prepare($conn, 'DELETE FROM guide WHERE id = ? LIMIT 1');
            if ($st) {
                mysqli_stmt_bind_param($st, 'i', $guideId);
                mysqli_stmt_execute($st);
                $aff = mysqli_stmt_affected_rows($st);
                mysqli_stmt_close($st);
                $_SESSION[$aff > 0 ? 'guide_flash_success' : 'guide_flash_error'] =
                    $aff > 0 ? 'Guide supprimé avec succès.' : 'Guide introuvable.';
            } else {
                $_SESSION['guide_flash_error'] = 'Suppression impossible pour le moment.';
            }
        }
    ...
    if ($action === 'save') {
        ...
        if ($titre === '' || $localisation === '') {
            $_SESSION['guide_flash_error'] = 'Titre et localisation sont obligatoires.';
            header('Location: ' . $redirect); exit;
        }
        if (!is_numeric($prixRaw)) {
            $_SESSION['guide_flash_error'] = 'Prix invalide.';
            header('Location: ' . $redirect); exit;
        }
        ...
        if ($guideId > 0) {
            if ($photoP !== '') {
                ...
                $sql = 'UPDATE guide SET titre=?,localisation=?,prix=?,capacite=?,date_debut=?,date_fin=?,description=?,inclus=?,photo_principale=?,photos_sec=?,statut=?,region_id=? WHERE id=? LIMIT 1';
                $st  = mysqli_prepare($conn, $sql);
                if ($st) {
                    mysqli_stmt_bind_param($st,'ssdisssssssii',$titre,$localisation,$prix,$capacite,$ddVal,$dfVal,$description,$inclusJs,$photoP,$photosSecJs,$statut,$regionId,$guideId);
                    mysqli_stmt_execute($st); mysqli_stmt_close($st);
                    $_SESSION['guide_flash_success'] = 'Guide mis à jour.';
                } else { $_SESSION['guide_flash_error'] = 'Modification impossible.'; }
            } else {
                // UPDATE without photo
                $sql = 'UPDATE guide SET titre=?,localisation=?,prix=?,capacite=?,date_debut=?,date_fin=?,description=?,inclus=?,statut=?,region_id=? WHERE id=? LIMIT 1';
                $st  = mysqli_prepare($conn, $sql);
                if ($st) {
                    mysqli_stmt_bind_param($st,'ssdisssssii',$titre,$localisation,$prix,$capacite,$ddVal,$dfVal,$description,$inclusJs,$statut,$regionId,$guideId);
                    mysqli_stmt_execute($st); mysqli_stmt_close($st);
                    $_SESSION['guide_flash_success'] = 'Guide mis à jour.';
                } else { $_SESSION['guide_flash_error'] = 'Modification impossible.'; }
            }
        } else {
            $sql = 'INSERT INTO guide (titre,localisation,prix,capacite,date_debut,date_fin,description,inclus,photo_principale,photos_sec,statut,region_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)';
            $st  = mysqli_prepare($conn, $sql);
            if ($st) {
                ...
                mysqli_stmt_execute($st); mysqli_stmt_close($st);
                $_SESSION['guide_flash_success'] = 'Guide ajouté avec succès.';
            } else { $_SESSION['guide_flash_error'] = 'Création impossible pour le moment.'; }
        }
        header('Location: ' . $redirect); exit;
    }
}
...
$editItem = null;
if ($editId > 0) {
    $sql = 'SELECT id,titre,localisation,prix,capacite,date_debut,date_fin,description,inclus,photo_principale,photos_sec,statut,region_id FROM guide WHERE id=? LIMIT 1';
    $st  = mysqli_prepare($conn, $sql);
    if ($st) {
...
$totalItems = 0;
$cst = mysqli_prepare($conn, 'SELECT COUNT(*) FROM guide' . $where);
if ($cst) {
...
$items = [];
$lst = mysqli_prepare($conn,'SELECT id,titre,localisation,prix,capacite,date_debut,date_fin,statut,photo_principale,region_id FROM guide'.$where.' ORDER BY id DESC LIMIT ? OFFSET ?');
if ($lst) {
...
$pageTitle   = 'Guides locaux';
$pageHeading = 'Guides locaux';
$activePage  = 'guide';
...
<main class="admin-content">
  <div class="content-wrap">
    ...
    <div class="page-body">
      ...
      <!-- FORM -->
      <div class="form-card">
        <div class="stat-label"><?php echo $editItem ? 'Modifier le guide' : 'Ajouter un guide'; ?></div>
        <form method="post" action="guide.php?q=<?php echo urlencode($search); ?>&page=<?php echo (int)$page; ?>" enctype="multipart/form-data">
          <input type="hidden" name="action"     value="save">
          <input type="hidden" name="guide_id"   value="<?php echo $editItem ? (int)$editItem['id'] : 0; ?>">
          ...
          <div class="form-grid">
            ...
            <div class="form-field" style="grid-column:1/-1;">
              ...
              <textarea name="description" class="form-textarea" placeholder="Description du guide..."><?php echo htmlspecialchars($editItem ? $editItem['description'] : ''); ?></textarea>
            </div>
          ...
          <div class="actions" style="margin-top:14px;">
            <button type="submit" class="btn-small btn-coral">
              <?php echo $editItem ? 'Enregistrer les modifications' : 'Ajouter le guide'; ?>
            </button>
            <a href="guide.php?q=<?php echo urlencode($search); ?>&page=<?php echo (int)$page; ?>" class="btn-small btn-soft">Annuler</a>
          </div>
        </form>
      </div><!-- /form-card -->
      <?php endif; ?>
      ...
      <div class="toolbar">
        <div style="display:flex;gap:12px;align-items:center;flex-wrap:wrap;">
            <a href="guide.php?add=1" class="btn-small btn-coral">+ Ajouter un guide</a>
            <form method="get" action="guide.php" class="search-form" style="margin:0;">
              ...
              <?php if ($search !== '' || $status !== ''): ?>
                <a href="guide.php" class="btn-small btn-soft">Réinitialiser</a>
              <?php endif; ?>
            </form>
        </div>
        <div class="muted">Total guides : <?php echo (int)$totalItems; ?></div>
      </div>

      <!-- STATUS FILTER PILLS -->
      <div style="margin:0 0 18px;">
        <?php admin_render_status_filter($status, 'guide.php', ['q' => $search]); ?>
      </div>
      ...
      <div class="popular-grid">
        <?php if (empty($items)): ?>
          <div class="mini-card" style="grid-column: 1/-1;">
            <h4>Aucun guide</h4>
            <p class="muted">Ajoutez votre premier guide.</p>
          </div>
        <?php else: ?>
          <?php foreach (array_slice($items, 0, 4) as $item): ?>
            <article class="pop-card">
              <div class="pop-card-img">
                <span class="pop-badge">Guide</span>
                <?php 
                  $img = $item['photo_principale'] ?: 'https://images.unsplash.com/photo-1501503060445-7382139d5381?auto=format&fit=crop&w=600&q=80';
              ...
              <div class="pop-card-body">
                ...
                <div class="pop-card-price">
                  ...
                  <a href="guide.php?edit=<?php echo $item['id']; ?>" class="pop-btn">
      ...
      <div class="table-wrap">
        <table class="data-table">
          ...
          <tbody>
            <?php if (empty($items)): ?>
              <tr><td colspan="8">Aucun guide trouvé.</td></tr>
            <?php else: ?>
                  ...
                  <td>
                    <div class="actions">
                      <a class="btn-small btn-soft"
                         href="guide.php?edit=<?php echo (int)$item['id']; ?>&q=<?php echo urlencode($search); ?>&page=<?php echo (int)$page; ?>" title="Modifier"><i class="bi bi-pencil-square"></i></a>
                      <form method="post" action="guide.php" class="inline-form"
                            onsubmit="return confirm('Supprimer ce guide ?');">
                        <input type="hidden" name="action"     value="delete">
                        <input type="hidden" name="guide_id"   value="<?php echo (int)$item['id']; ?>">
                        <input type="hidden" name="q"          value="<?php echo htmlspecialchars($search); ?>">
                        <input type="hidden" name="page"       value="<?php echo (int)$page; ?>">
        ...
        <div class="pagination">
          <?php for ($p = 1; $p <= $totalPages; $p++): ?>
            <a class="btn-small <?php echo $p === $page ? 'btn-coral' : 'btn-soft'; ?>"
               href="guide.php?page=<?php echo (int)$p; ?>&q=<?php echo urlencode($search); ?>">
              <?php echo (int)$p; ?>
            </a>
          <?php endfor; ?>
        </div>
