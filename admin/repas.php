...
// ---------- Upload directory ----------
$uploadBase = realpath(__DIR__ . '/..') . DIRECTORY_SEPARATOR . 'uploads' . DIRECTORY_SEPARATOR . 'repas';
...
$flashSuccess = '';
$flashError   = '';
if (!empty($_SESSION['repas_flash_success'])) { $flashSuccess = (string)$_SESSION['repas_flash_success']; unset($_SESSION['repas_flash_success']); }
if (!empty($_SESSION['repas_flash_error']))   { $flashError   = (string)$_SESSION['repas_flash_error'];   unset($_SESSION['repas_flash_error']); }
...
function moveUploadedImage(string $name, int $idx = -1): string {
    ...
    $file = uniqid('rep_', true) . '.' . $ext;
    if (!move_uploaded_file($tmp, $uploadBase . DIRECTORY_SEPARATOR . $file)) return '';
    return 'uploads/repas/' . $file;
}

// ---------- POST handling ----------
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $action    = (string)($_POST['action']     ?? '');
    $repasId   = (int)($_POST['repas_id']      ?? 0);
    $qReturn   = trim((string)($_POST['q']     ?? ''));
    $pgReturn  = max(1, (int)($_POST['page']   ?? 1));
    $redirect  = 'repas.php?q=' . urlencode($qReturn) . '&page=' . $pgReturn;

    // DELETE
    if ($action === 'delete') {
        if ($repasId <= 0) {
            $_SESSION['repas_flash_error'] = 'Repas invalide.';
        } else {
            $st = mysqli_prepare($conn, 'DELETE FROM repas WHERE id = ? LIMIT 1');
            if ($st) {
                mysqli_stmt_bind_param($st, 'i', $repasId);
                mysqli_stmt_execute($st);
                $aff = mysqli_stmt_affected_rows($st);
                mysqli_stmt_close($st);
                $_SESSION[$aff > 0 ? 'repas_flash_success' : 'repas_flash_error'] =
                    $aff > 0 ? 'Repas supprimé avec succès.' : 'Repas introuvable.';
            } else {
                $_SESSION['repas_flash_error'] = 'Suppression impossible pour le moment.';
            }
        }
    ...
    if ($action === 'save') {
        ...
        if ($titre === '' || $localisation === '') {
            $_SESSION['repas_flash_error'] = 'Titre et localisation sont obligatoires.';
            header('Location: ' . $redirect); exit;
        }
        if (!is_numeric($prixRaw)) {
            $_SESSION['repas_flash_error'] = 'Prix invalide.';
            header('Location: ' . $redirect); exit;
        }
        ...
        if ($repasId > 0) {
            if ($photoP !== '') {
                ...
                $sql = 'UPDATE repas SET titre=?,localisation=?,prix=?,capacite=?,date_debut=?,date_fin=?,description=?,inclus=?,photo_principale=?,photos_sec=?,statut=?,region_id=? WHERE id=? LIMIT 1';
                $st  = mysqli_prepare($conn, $sql);
                if ($st) {
                    mysqli_stmt_bind_param($st,'ssdisssssssii',$titre,$localisation,$prix,$capacite,$ddVal,$dfVal,$description,$inclusJs,$photoP,$photosSecJs,$statut,$regionId,$repasId);
                    mysqli_stmt_execute($st); mysqli_stmt_close($st);
                    $_SESSION['repas_flash_success'] = 'Repas mis à jour.';
                } else { $_SESSION['repas_flash_error'] = 'Modification impossible.'; }
            } else {
                // UPDATE without photo
                $sql = 'UPDATE repas SET titre=?,localisation=?,prix=?,capacite=?,date_debut=?,date_fin=?,description=?,inclus=?,statut=?,region_id=? WHERE id=? LIMIT 1';
                $st  = mysqli_prepare($conn, $sql);
                if ($st) {
                    mysqli_stmt_bind_param($st,'ssdisssssii',$titre,$localisation,$prix,$capacite,$ddVal,$dfVal,$description,$inclusJs,$statut,$regionId,$repasId);
                    mysqli_stmt_execute($st); mysqli_stmt_close($st);
                    $_SESSION['repas_flash_success'] = 'Repas mis à jour.';
                } else { $_SESSION['repas_flash_error'] = 'Modification impossible.'; }
            }
        } else {
            $sql = 'INSERT INTO repas (titre,localisation,prix,capacite,date_debut,date_fin,description,inclus,photo_principale,photos_sec,statut,region_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)';
            $st  = mysqli_prepare($conn, $sql);
            if ($st) {
                ...
                mysqli_stmt_execute($st); mysqli_stmt_close($st);
                $_SESSION['repas_flash_success'] = 'Repas ajouté avec succès.';
            } else { $_SESSION['repas_flash_error'] = 'Création impossible pour le moment.'; }
        }
        header('Location: ' . $redirect); exit;
    }
}
...
$editItem = null;
if ($editId > 0) {
    $sql = 'SELECT id,titre,localisation,prix,capacite,date_debut,date_fin,description,inclus,photo_principale,photos_sec,statut,region_id FROM repas WHERE id=? LIMIT 1';
    $st  = mysqli_prepare($conn, $sql);
    if ($st) {
...
$totalItems = 0;
$cst = mysqli_prepare($conn, 'SELECT COUNT(*) FROM repas' . $where);
if ($cst) {
...
$items = [];
$lst = mysqli_prepare($conn,'SELECT id,titre,localisation,prix,capacite,date_debut,date_fin,statut,photo_principale,region_id FROM repas'.$where.' ORDER BY id DESC LIMIT ? OFFSET ?');
if ($lst) {
...
$pageTitle   = 'Repas maison';
$pageHeading = 'Repas maison';
$activePage  = 'repas';
...
<main class="admin-content">
  <div class="content-wrap">
    ...
    <div class="page-body">
      ...
      <!-- FORM -->
      <div class="form-card">
        <div class="stat-label"><?php echo $editItem ? 'Modifier le repas' : 'Ajouter un repas'; ?></div>
        <form method="post" action="repas.php?q=<?php echo urlencode($search); ?>&page=<?php echo (int)$page; ?>" enctype="multipart/form-data">
          <input type="hidden" name="action"     value="save">
          <input type="hidden" name="repas_id"   value="<?php echo $editItem ? (int)$editItem['id'] : 0; ?>">
          ...
          <div class="form-grid">
            ...
            <div class="form-field">
              ...
              <input type="text" name="titre" class="form-input"
                     ...
                     placeholder="Nom du repas" required>
            </div>
            ...
            <div class="form-field" style="grid-column:1/-1;">
              ...
              <textarea name="description" class="form-textarea" placeholder="Description du repas..."><?php echo htmlspecialchars($editItem ? $editItem['description'] : ''); ?></textarea>
            </div>
          ...
          <div class="actions" style="margin-top:14px;">
            <button type="submit" class="btn-small btn-coral">
              <?php echo $editItem ? 'Enregistrer les modifications' : 'Ajouter le repas'; ?>
            </button>
            <a href="repas.php?q=<?php echo urlencode($search); ?>&page=<?php echo (int)$page; ?>" class="btn-small btn-soft">Annuler</a>
          </div>
        </form>
      </div><!-- /form-card -->
      <?php endif; ?>
      ...
      <div class="toolbar">
        <div style="display:flex;gap:12px;align-items:center;flex-wrap:wrap;">
            <a href="repas.php?add=1" class="btn-small btn-coral">+ Ajouter un repas</a>
            <form method="get" action="repas.php" class="search-form" style="margin:0;">
              ...
              <?php if ($search !== '' || $status !== ''): ?>
                <a href="repas.php" class="btn-small btn-soft">Réinitialiser</a>
              <?php endif; ?>
            </form>
        </div>
        <div class="muted">Total repas : <?php echo (int)$totalItems; ?></div>
      </div>

      <!-- STATUS FILTER PILLS -->
      <div style="margin:0 0 18px;">
        <?php admin_render_status_filter($status, 'repas.php', ['q' => $search]); ?>
      </div>
      ...
      <div class="popular-grid">
        <?php if (empty($items)): ?>
          <div class="mini-card" style="grid-column: 1/-1;">
            <h4>Aucun repas</h4>
            <p class="muted">Ajoutez votre premier repas.</p>
          </div>
        <?php else: ?>
          <?php foreach (array_slice($items, 0, 4) as $item): ?>
            <article class="pop-card">
              <div class="pop-card-img">
                ...
                <?php 
                  $img = $item['photo_principale'] ?: 'https://images.unsplash.com/photo-1504674900247-0877df9cc836?auto=format&fit=crop&w=600&q=80';
              ...
              <div class="pop-card-body">
                ...
                <div class="pop-card-price">
                  ...
                  <a href="repas.php?edit=<?php echo $item['id']; ?>" class="pop-btn">
      ...
      <div class="table-wrap">
        <table class="data-table">
          ...
          <tbody>
            <?php if (empty($items)): ?>
              <tr><td colspan="8">Aucun repas trouvé.</td></tr>
            <?php else: ?>
                  ...
                  <td>
                    <div class="actions">
                      <a class="btn-small btn-soft"
                         href="repas.php?edit=<?php echo (int)$item['id']; ?>&q=<?php echo urlencode($search); ?>&page=<?php echo (int)$page; ?>" title="Modifier"><i class="bi bi-pencil-square"></i></a>
                      <form method="post" action="repas.php" class="inline-form"
                            onsubmit="return confirm('Supprimer ce repas ?');">
                        <input type="hidden" name="action"     value="delete">
                        <input type="hidden" name="repas_id"   value="<?php echo (int)$item['id']; ?>">
                        <input type="hidden" name="q"          value="<?php echo htmlspecialchars($search); ?>">
                        <input type="hidden" name="page"       value="<?php echo (int)$page; ?>">
        ...
        <div class="pagination">
          <?php for ($p = 1; $p <= $totalPages; $p++): ?>
            <a class="btn-small <?php echo $p === $page ? 'btn-coral' : 'btn-soft'; ?>"
               href="repas.php?page=<?php echo (int)$p; ?>&q=<?php echo urlencode($search); ?>">
              <?php echo (int)$p; ?>
            </a>
          <?php endfor; ?>
        </div>
