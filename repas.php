...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_SESSION['user_id'])) {
    ...
    header("Location: login.php?redirect=" . urlencode("repas.php?id=$id"));
    exit;
}
...
// Fetch repas
$st = mysqli_prepare($conn, "SELECT * FROM repas WHERE id = ? LIMIT 1");
...
// Fetch avis
$avis_list = [];
$st_avis = mysqli_prepare($conn, "SELECT a.*, u.nom as user_nom FROM avis a JOIN utilisateur u ON a.utilisateur_id = u.id WHERE a.repas_id = ? ORDER BY a.created_at DESC");
if ($st_avis) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_SESSION['user_id'])) {
    ...
    if (empty($date)) {
        ...
        $stmt = mysqli_prepare($conn, "INSERT INTO reservations (utilisateur_id, repas_id, date_debut, nb_personnes, statut) VALUES (?, ?, ?, ?, 'en_attente')");
        if ($stmt) {
...
function formatImagePath($path) {
    ...
    if (strpos($path, 'uploads/repas/') === 0) return $path;
    return 'uploads/repas/' . ltrim($path, '/');
}
...
<div class="product-container">
  <div class="left-column">
    ...
    <div class="description-block">
      <h3>Description du repas</h3>
      <p><?= htmlspecialchars($item['description']) ?></p>
      
      <?php if(!empty($inclus)): ?>
      <h3 style="margin-top:32px;">Au menu</h3>
  ...
  <div class="booking-card">
    ...
    <form method="post" action="repas.php?id=<?= $id ?>">
      ...
      <div class="form-group">
        ...
        <textarea name="message" class="input-field" rows="3" placeholder="Demandes spéciales, allergies..."></textarea>
      </div>
